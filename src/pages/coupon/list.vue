    ...
    <el-table :data="tableData" stripe border style="width: 100%" v-loading="loading">
        ...
        <el-table-column label="操作" width="250px" align="center">
            <template #default="scope">
                ...
                <el-popconfirm v-if="scope.row.statusText != '领取中'" title="是否要删除该优惠卷" confirm-button-text="确认"
                    cancel-button-text="取消" @confirm="handleDelete(scope.row.id)">
                    <template #reference>
                        <el-button text type="primary" size="small">删除</el-button>
                    </template>
                </el-popconfirm>
                <el-popconfirm v-if="scope.row.statusText == '领取中'" title="是否让该优惠卷失效" confirm-button-text="确认"
    ...
    <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
        <el-form :model="form" ref="formRef" :rules="rules" label-width="100px" :inline="false">
            <el-form-item label="优惠卷名称" prop="name">
                <el-input v-model="form.name" placeholder="优惠卷名称" style="width:50%;"></el-input>
            </el-form-item>
            ...
            <el-form-item label="面值" prop="value">
                ...
                <el-input v-model="form.value" placeholder="面值" style="width:50%;"></el-input>
                <template #append>{{ form.type ? '折' : '元' }}</template>
            </el-form-item>
            <el-form-item label="发行量" prop="total">
                <el-input-number v-model="form.total" :min="0" :max="1000000">
                </el-input-number>
            </el-form-item>
...
const {
    ...
    handleEdit } = useInitForm({
        form: {
            ...
            name: [{ required: true, message: '优惠卷名称不能为空', trigger: 'blur' },],
        },
        getData,
        update: updateCoupon,
        create: createCoupon,
...
.inactive {
    @apply border-gray-200 bg-green-400 text-gray-400;
}
</style>